...
os.system('copy %s %s' % (os.path.join(BASE_DIR, 'models/model.py'), LOG_DIR))  # bkp of model def
os.system('copy %s %s' % (os.path.join(BASE_DIR, 'train.py'), LOG_DIR))  # bkp of train procedure
    ...
    with tf.Graph().as_default():
        ...
        def train_one_epoch(epoch_num):
            ...
            for j in range(num_batch):
                ...
                printout(flog, 'Batch: %d, loss: %f, grouperr: %f, same: %f, diff: %f, pos: %f' % (
                j, loss_val, grouperr_val, total_same, (diff_val / diff_cnt_val), pos_val / pos_cnt_val))
