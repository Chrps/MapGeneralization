...
import SGPN.provider as provider
from SGPN.utils.test_utils import *
from SGPN.models import model
...
parser.add_argument('--input_list', type=str, default='data/generated_pointclouds/valid_hdf5_file_list.txt', help='Validation data list')
    ...
    with tf.Session(config=config) as sess:
        ...
        for shape_idx in range(len_pts_files):
            ...
            for j in range(num_data):
                ...
                #npy_loaded = np.load(r"D:\University Stuff\Master's Thesis\Programming Projects\SGPN\data\generated_pointclouds\annotations\room_10.npy")
                '''
                for idx, point in enumerate(npy_loaded):
                    if ptsclassification_val[idx] == 0:
                        color = 'b'
                    elif ptsclassification_val[idx] == 1:
                        color = 'r'
                    else:
                        color = 'g'
